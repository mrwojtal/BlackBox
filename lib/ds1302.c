...
/// @brief Function used to initialize GPIOs ocntained in given pointer to DS1302Instance struct.
...
/// @param ds1302 struct for ds1302 instance
/// @param time time to set in format [year, month, date, day, hour, minutes, seconds] ex. [24, 5, 28, 3, 15, 30, 10]
...
/// @brief Function fills given char buffer with time in format HH;MM;SS.
/// @param ds1302 struct for ds1302 instance
/// @param buffer buffer to be filled with time string
...
/// @brief Function fills given char buffer with time in format HH;MM.
/// @param ds1302 struct for ds1302 instance
/// @param buffer buffer to be filled with time string
/// @param buffer_size size of given buffer
void ds1302_get_time_hm(DS1302Instance* ds1302, char* buffer, uint8_t buffer_size) {
    ...
    if (hour < 10 && minutes < 10){
        snprintf(buffer, buffer_size, "0%d;0%d", hour, minutes);
    }
    else if (hour >= 10 && minutes >= 10) {
        snprintf(buffer, buffer_size, "%d;%d", hour, minutes);
    }
    else if (hour >= 10 && minutes < 10) {
        snprintf(buffer, buffer_size, "%d;0%d", hour, minutes);
    }
    else if (hour < 10 && minutes >= 10) {
        snprintf(buffer, buffer_size, "0%d;%d", hour, minutes);
    }
}

/// @brief Function fills given char buffer with date in format YYYY.MM.DD
/// @param ds1302 struct for ds1302 instance
/// @param buffer buffer to be filled with date string
/// @param buffer_size size of given buffer
...
/// @brief Function fills given char buffer with date and time in format YYYY.MM.DD_HH;MM;SS
/// @param ds1302 struct for ds1302 instance
/// @param buffer buffer to be filled with date and time string
/// @param buffer_size size of given buffer
void ds1302_get_date_and_time(DS1302Instance* ds1302, char* buffer, uint8_t buffer_size) {
    char tmp_buff[TIME_RESPONSE_BUFFER_SIZE];
    ds1302_get_date(ds1302, buffer, buffer_size);
    ds1302_get_time(ds1302, tmp_buff, TIME_RESPONSE_BUFFER_SIZE);
    strcat(buffer, "_");
